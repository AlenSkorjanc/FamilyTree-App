...
export type PersonNodeData = { person: Person; onAddRelative?: (kind: RelativeKind) => void; isAlias?: boolean }
...
export function PersonNode({ data, selected }: NodeProps<PersonFlowNode>) {
  ...
  return (
    <article className={`person-node person-node-${genderTone} ${selected ? 'selected' : ''} ${data.isAlias ? 'person-node-alias' : ''}`} aria-label={fullName(data.person)}>
      ...
      <Handle type="source" position={Position.Left} id="partner-source-left" />
      <PersonAvatar person={data.person} />
      <div className="person-node-copy">
        <strong>{fullName(data.person)}</strong>
        <small>{lifeYears(data.person, t('born'), language)}</small>
      </div>
      {selected && !data.isAlias && data.onAddRelative && <div className="node-quick-add nodrag nowheel">
      ...
      <Handle type="target" position={Position.Right} id="partner-target-right" />
